<?php
	...
	if(!isset($_SESSION['login']) || !($_SESSION['tipo'] == 3 || $_SESSION["tipo"] == 4)){
		setcookie("redirect","atividades/pendentes.php", time() + (86400 * 30), "/");
...
<html lang="pt-br">
<head>
	<title>SIGEP - Tarefas Pendentes</title>
	<meta charset="utf-8"/>
...
<span>Sistema de Gerenciamento de Projeto</span></div>
	...
	<div id="or"><a href="../selectproj.php">selecionar projeto</a><a href="../">inicio</a><a href="../atividades">tarefas</a><span>pendentes</span><a href="../atividades" class="voltar">voltar</a></div>
	<section id="conteudo">
		<h3 class='titulo'>Tarefas Pendentes</h3>
		<article>
			<?php
			require_once("../../_includes/conexao.php");
			$sql = "SELECT trtitvar,trdtfcha,trcodint FROM tarefas WHERE trstaint = 0 AND trpsfint = ".$_SESSION["pscod"]." AND trprfint = ".$_SESSION["projeto"]." ORDER BY trdtfcha DESC";
			...
			if($x > 0){ ?>
				<table class="tb">
					...
					while ($res = $quer->fetch_object()):
						?>
                        <tr>
                            <td><a href="visualizar.php?src=pendentes&id=<?php echo $res->trcodint; ?>" target="_blank"><?php echo $res->trtitvar; ?></a></td>
				...
				?>
				<div class="boaMsg"><img src="../../_icones/ok.png">Nenhuma atividade pendente!<span>x</span></div>				
				<?php
			}
			?>
		</article>		
